    ...
    <div className="max-w-4xl mx-auto my-8 px-4">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200/80 p-6 sm:p-10">
        ...
        <form onSubmit={handleSubmit}>
          {/* PDF Drag and Drop Zone with #F0F9FF hover state & keyboard support */}
          <div 
            role="button"
            tabIndex={0}
            aria-label="Upload PDF resume file drag and drop zone"
            className={`relative border-2 border-dashed rounded-2xl p-8 sm:p-12 text-center transition-all duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 ${
              dragActive 
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center space-y-3 pointer-events-none">
              <div className="w-16 h-16 bg-gradient-to-br from-blue-100 to-cyan-100 text-blue-600 rounded-2xl flex items-center justify-center mb-1 shadow-xs border border-blue-200/60">
                <svg className="w-8 h-8 text-cyan-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <p className="text-xs text-slate-600 font-medium">PDF documents up to 10MB</p>
            </div>
          </div>
          ...
          {file && (
            <div className="mt-5 flex items-center justify-between p-4 bg-gradient-to-r from-blue-50/80 via-cyan-50/80 to-blue-50/80 border border-cyan-200 rounded-2xl shadow-sm">
              <div className="flex items-center space-x-3.5 truncate">
                <div className="p-2.5 bg-red-500 text-white rounded-xl shadow-xs" aria-hidden="true">
                ...
                <div className="truncate">
                  ...
                  <p className="text-xs text-slate-600 font-medium">{(file.size / (1024 * 1024)).toFixed(2)} MB • PDF Document</p>
                </div>
              </div>
              <button 
                type="button" 
                onClick={(e) => { e.stopPropagation(); setFile(null); }}
                className="text-slate-500 hover:text-red-500 font-semibold text-xs px-3 py-1.5 rounded-lg border border-transparent hover:border-red-200 hover:bg-red-50 transition-all flex items-center gap-1 cursor-pointer"
          ...
          <div className="mt-6">
            <label htmlFor="job-description" className="block text-sm font-extrabold text-[#0F172A] mb-2">
              Target Job Description <span className="text-slate-500 font-normal text-xs">(Optional for targeted Skill Gap matching)</span>
            </label>
            <textarea
              id="job-description"
              ...
              className="w-full p-4 border border-slate-300 rounded-2xl shadow-xs focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-cyan-400 text-sm text-slate-800 placeholder-slate-500 bg-white transition-all"
            />
          </div>

          {/* Error Message */}
          {error && (
            <div role="alert" className="mt-4 p-4 bg-red-50 border border-red-200 text-red-700 text-sm rounded-2xl flex items-center gap-2 font-medium">
